...
import { Mail, Lock, BookCheck, Code } from "lucide-react"
import { AuthService } from "@/lib/auth-service"

export default function TrainingAssessmentsLoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (user) {
      if (user.role !== "training-assessments" && user.role !== "admin") {
        setError("Access denied. This account does not have training & assessments access.")
        return
      }
      ...
      if (user.role === "admin") {
        ...
        navigate("/training-assessments/dashboard")
      }
    } else {
      ...
      >
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">
            Training & Assessments Portal
          </h1>
          <p className="text-gray-600">Manage training programs and interview prep</p>
        </div>
        ...
        <Card className="border border-gray-200 shadow-sm bg-white p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-600">Sign in to your training & assessments account</p>
          </div>
